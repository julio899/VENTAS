...
class Facturacion extends CI_Controller{
	...
	public function index(){
		...
		$cant_pedidos_nuevos=$this->data_complemento->cantidad_pedidos_nuevos();
					...
					$tabla="<table class=\"table table-hover table-condensed tbl-ped-env\"><tr><th>Nro.Pedido</th><th>COMPA&Ntilde;IA</th><th>ZONA</th><th>COD. CLIENTE</th><th>Raz&oacute;n Social</th><th>FECHA</th><th>TIPO</th><th>Detalle</th></tr>";
				for ($i=0; $i < count($pedidos); $i++) { 
					...
					if($pedidos[$i]['status']=="I"){
						$tabla.= "<tr style=\"background-color:#CEECF5\"><td>".$pedidos[$i]['id']."</td><td>".strtoupper($this->data_complemento->get_compaTXT($pedidos[$i]['compa']))."</td><td>".$pedidos[$i]['zona']."</td><td>".$pedidos[$i]['codcte']."</td><td>".$cliente['razsoc']."</td><td>".date_format(date_create($pedidos[$i]['fecha']),"d-m-Y g:i A")."</td><td>".$tipo."</td><td><a class=\"btn btn-info\" href=\"".base_url().index_page()."/ventas/detallar_pedido/".$pedidos[$i]['id']."\"><i class=\"icon-search\" title=\"Mostrar Detalles\"></i> Ver</a></td></tr>";
			
					}else{
						$tabla.= "<tr><td>".$pedidos[$i]['id']."</td><td>".strtoupper($this->data_complemento->get_compaTXT($pedidos[$i]['compa']))."</td><td>".$pedidos[$i]['zona']."</td><td>".$pedidos[$i]['codcte']."</td><td>".$cliente['razsoc']."</td><td>".date_format(date_create($pedidos[$i]['fecha']),"d-m-Y g:i A")."</td><td>".$tipo."</td><td><a class=\"btn btn-info\" href=\"".base_url().index_page()."/ventas/detallar_pedido/".$pedidos[$i]['id']."\"><i class=\"icon-search\" title=\"Mostrar Detalles\"></i> Ver</a></td></tr>";
					}
