...
function enviar_email_vendedor($id_vendedor,$data){
	...
	$this->email->cc("jomaor671967@hotmail.com");
...
return  array('cabecera' => $this->temporal,'productos'=> $this->productos_del_pedido($idp));
...
function todos_pedidos_usuario($idu){
    ...
    $query=$this->db->query($sql);  
        foreach ($query->result() as $row)
                {
                    $this->temporal[]=array('id'=>$row->id,'compa'=>$row->compa,'zona'=>$row->zona,'codcte'=>$row->codcte,'fecha'=>$row->fecha,'nota'=>$row->nota,'status'=>$row->status,'tipo'=>$row->tipo);
                }
return $this->temporal;
}//ultimo pedido  del usuario
...
switch ($codCompa) {
	...
	$this->query = $this->db->query("SELECT * FROM  `01_cpc` WHERE  `codcte` LIKE CONVERT( _utf8 '$codCte' USING latin1 ) COLLATE latin1_swedish_ci LIMIT 1");
